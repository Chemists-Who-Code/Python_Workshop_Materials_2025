...
The miniconda distribution comes with a default package manager called `conda`. Note all packages you may want are registered with the conda package manager, so it is useful to install the other common package manager `pip`.  
